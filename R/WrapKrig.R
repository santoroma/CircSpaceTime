...
#'
#' \code{WrapKrig} function computes the Kriging prediction for circular spatial data
...
)
{
  ...
  if (corr_fun == "kappa_matern") {
    kappa_matern <- WrapSp_out$kappa_matern
  }
  n	<- nrow(k)
  ...
  if (length(which(names(WrapSp_out[[1]]) == "beta")) == 1) {
    pp <- unlist(WrapSp_out)
    beta <- as.numeric(pp[regexpr("beta",names(pp)) == 1])
    rm(pp)
    out <- WrapKrigCpp_nugget(sigma2, alpha, rho, beta, k, n, nsample, H_tot, nprev, x_oss, corr_fun, kappa_matern)
  } else {
    out <- WrapKrigCpp(sigma2,	alpha, rho, k, n, nsample,	H_tot,nprev, x_oss, corr_fun, kappa_matern)
}
