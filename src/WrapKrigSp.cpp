...
List WrapKrigSpCpp(
  ...
  *****************************************/
  for(KrigI=0;KrigI<nsample;KrigI++)
  {
    ...
    // inverse correlation matrix of the observed data
    ...
    // correlation matrix between observed and non-observed data
    if(corr_fun == "exponential"){
      for(i=0;i<nprev;i++)
      {
        for(h=0;h<n;h++)
    ...
    // compute the conditional mean and covariance matrix
    // of the non observed data
