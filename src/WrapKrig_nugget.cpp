...
// [[Rcpp::export]]
List WrapKrigCpp_nugget(
    NumericVector sigma2, NumericVector alpha, NumericVector rho, NumericVector beta,
    IntegerMatrix k,
  ...
  for(KrigI=0;KrigI<nsample;KrigI++)
  {
    ...
    arma::mat Ident(Cor_inv.n_rows,Cor_inv.n_cols,arma::fill::eye);
    Cor_inv = Cor_inv + beta[KrigI]*Ident;
    ...
    for(i=0;i<nprev;i++)
    {
      ...
      V_prev[i] = sigma2[KrigI]*(1. + beta[KrigI]) - V_prev[i];
    }
    for(i=0;i<nprev;i++)
    {
