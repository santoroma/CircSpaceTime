...
// [[Rcpp::export]]
List WrapSpRcpp_nugget(
    int ad_start, int ad_end, double ad_esp,
    int burnin, int thin, int iter_1, int iter_2,
    int n_j,
    NumericVector prior_alpha, NumericVector prior_rho, NumericVector prior_sigma2, NumericVector prior_beta,
    double sdrho, double sdsigma2, double sdbeta,
    double alpha, double rho, double sigma2, double beta, IntegerVector k,
  ...
  int h,j;
  arma::mat Ident(H.n_rows,H.n_cols,arma::fill::eye);
  ...
  mean_sp[2] = beta + R::rnorm(0.0,0.1);
  ...
  NumericVector sigma2_out_add(nSamples_save), rho_out_add(nSamples_save), beta_out_add(nSamples_save);
  ...
  //   Progress p(iter_2*BurinOrThin, true);
  //  double progress = 0.0;
  //std::cout << std::endl;
  int k_MH;
  double UnifSample;
  ...
  for(iMCMC2=0;iMCMC2<iter_2;iMCMC2++)
  {
    ...
    //     else Rcout << " ";
    //   }
    //   Rcout << "] " << int(progress * 100.0) << " %\r" << std::flush;
    //   progress = (iMCMC2 + 1)/iter_2;
    // }
    ...
    for(iMCMC3=0;iMCMC3<BurinOrThin;iMCMC3++)
    {
      ...
      ******************/
      ...
      sim_sp[2] = log(beta/(prior_beta[0]-beta));
      ...
      beta_p  = exp(sim_sp_p[2])*prior_beta[0]/(1. + exp(sim_sp_p[2]));

      //    Rcout << "sig_p = "<<sigma2_p<<std::endl;
      //    Rcout << "beta_p = "<<beta_p<<std::endl;
      //        Rcout << "rhi_p = "<<rho_p<<std::endl;
      //        Rcout << "rhi = "<<rho<<std::endl;
      //        Rcout << "min = "<<prior_rho[0]<<std::endl;
      //        Rcout << "max = "<<prior_rho[1]<<std::endl;
      //        Rcout << "it = "<<iterations<<std::endl;
      ...
      Cor_inv_p = Cor_inv_p + beta_p*Ident;
      logdet_cor_p = 0.0;
      ...
      Cor_inv_p = arma::inv_sympd(0.5*(Cor_inv_p + Cor_inv_p.t()));

      // inverse gamma distribution for sigma2
      ...
      // beta distribution for beta ,
      Pbeta       = (prior_beta[1]-1.0)*log(beta)+(prior_beta[2]-1.0)*log(1.0-beta)+   sim_sp[2]-2.0*log(1.0+exp(sim_sp[2]));
      Pbeta_p     = (prior_beta[1]-1.0)*log(beta_p)+(prior_beta[2]-1.0)*log(1.0-beta_p)+sim_sp_p[2]-2.0*log(1.0+exp(sim_sp_p[2]));
      ...
      alpha_star= std::min(1.0,exp( MH_N_sp+Psigma2_p+ Prho_p+ Pbeta_p - (MH_D_sp+Psigma2+ Prho + Pbeta) ));
    ...
    ******************/
    //  }
    //   progress = (iMCMC3 + 1) * (iMCMC2 + 1) / (iter_2 * BurinOrThin);
  }
