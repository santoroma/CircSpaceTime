...
# Introduction
CircSpaceTime is an, under construction, R package implementing spatial and spatio-temporal models for circular data. As a first step we start with a purely spatial setting, as initially introduced in @Jona2013.
...
require(ggmap)
ggmap(maplayer <-get_map(location = c(min(storm1$Lon), min(storm1$Lat), max(storm1$Lon), max(storm1$Lat)), zoom = 6),
...
save(maplayer,file = "maplayer.RData")
```
...
train1 <- ggmap(get_map(location = c(min(train$Lon), min(train$Lat), 
                                     max(train$Lon), max(train$Lat)), 
                        zoom = 8), 
                base_layer = ggplot(aes(x = Lon, y = Lat, z = Hm0,
                                        fill = Hm0),
                                    data = train)) +
...
test1 <- ggmap(get_map(location = c(min(test$Lon), min(test$Lat),
                                    max(test$Lon), max(test$Lat)), 
                       zoom = 8),
               base_layer = ggplot(aes(x = Lon, y = Lat, z = Hm0,
                                       fill = Hm0),
                                   data = test)) +
...
save(Pred.calm, file = "Pred.calm.RData")
```
