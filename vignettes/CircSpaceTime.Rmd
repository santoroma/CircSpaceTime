...
# Introduction
CircSpaceTime is an, under construction, R package implementing spatial and spatio-temporal models for circular data. As a first step we start with a purely spatial setting, as initially introduced in \cite{Jona2013}.

There are different approaches to specify valid  circular distributions, see for example \cite{Jammalamadaka2001}. Here we focus on two methods that allow to built a circular distribution starting from  a linear one, namely the wrapping, and the projection.   Under both methods, the resulting distribution has a  complex functional form but introducing a suitable latent variable, the joint distribution of observed and latent variables are easy to handle in a fully Bayesian framework.
...
The wrapping approach can be easily  extended to a multivariate setting \cite{Jona2013}. Let $\mathbf{Y} =(Y_1,\dots, Y_p)^{\prime}$ be a $p$-variate vector with pdf $f_{\mathbf{Y}}(\cdot| \boldsymbol{\psi})$, then  $\boldsymbol{\Theta}=(\Theta_1,\dots, \Theta_p)^{\prime}$, with 
...
Since the  period of the tangent   is $\pi$,  inversion of this function, to obtain $\Theta$,  requires some care.   A common choice is the $\mbox{atan}^*$, formally defined in \cite{Jammalamadaka2001}, pag. 13, that takes into account the signs of $Y_1$ and $Y_2$ to determine   the right portion of the unit circle where $\Theta$ is located.    
...
The projection approach can be easily adapted to obtain a distribution for multivariate circular variable \cite{wang2014}. If $\mathbf{Y}$ is a $2p-$variate linear variable, a $p-$variate vector of (projected)  circular variables is obtained with the following transformation:
...
It is well known that the MCMC  tends to mix really slow (\cite{Andrieu2010}) when latent variable are involved. To speed up the convergence, we try to find an optimal proposal distribution for the Metropolis step using the algorithm described in  \cite{Robert2009}, page 258. With the goal of speeding up the MCMC convergence, is suggested to decrease the dimension of the parameters space, that is, do as much marginalization as possible (\cite{Banerjee2014}). 
The core estimation is based,  mostly, on loops with thounsands iterations. To improve performances \cite{wickham2015} we implemented everything in \emph{C$++$} and using \texttt{Rcpp} package we can simplify the integration between C$++$ and  R codes \cite{Eddelbuettel2011}. In particular we used the \texttt{RcppArmadillo} package \cite{Eddelbuettel2014} that implement the Armadillo matrix library for it simplicity and elegance \cite{Eddelbuettel2017},  although the \texttt{RcppEigen} is a bit faster \cite{Eddelbuettel2015}.
For a fast multiple chain estimations we used \texttt{doParallel} package \cite{doParallel}.


# Estimation of a model based on the Wrapped Gaussian
We now consider an example based on  wave directions (and heights). These data are obtained as outputs from a deterministic computer model implemented by Istituto Superiore per la Protezione e la Ricerca Ambientale (ISPRA) \cite{inghilesi2016}. 
...
```{r label="load", message=FALSE, echo=TRUE, warning=FALSE}
...
```{r label="plot1", message=FALSE, echo=FALSE, warning=FALSE, cache=TRUE}
require(ggmap)
ggmap(get_map(location = c(min(storm1$Lon), min(storm1$Lat), max(storm1$Lon), max(storm1$Lat)), zoom = 6),
...
```{r label0"stirm1$Dmr$, message=FALSE, echo=TRUE, warning=FALSE}
 storm1$Dmr<-storm1$Dm*pi/180
```

We plot the samples in terms of the sea state

```{r label="rosediag1", message=FALSE, echo=TRUE, warning=FALSE}
require(gridExtra)

r1<-rose_diag(storm1$Dmr[storm1$state=="calm"],bins=15,col=3,template="wind_rose")+
  ggtitle("Calm")
r2<-rose_diag(storm1$Dmr[storm1$state=="transition"],bins=15,col="yellow",template="wind_rose")+
  ggtitle("Transition")
r3<-rose_diag(storm1$Dmr[storm1$state=="storm"],bins=15,col=2,template="wind_rose")+
  ggtitle("Storm")
...
```{r label="storm2", message=FALSE, echo=TRUE, warning=FALSE}
...
```{r label="traintest1", message=FALSE, echo=TRUE, warning=FALSE,cache=TRUE}
...
```{r label="plot2",message=FALSE,echo=FALSE, warning=FALSE, cache=TRUE}
 
train1<-ggmap(get_map(location = c(min(train$Lon), min(train$Lat), max(train$Lon), max(train$Lat)), zoom = 8),
      base_layer = ggplot(aes(x = Lon, y = Lat, z = Hm0,
                              fill = Hm0),
                          data = train)) +
  geom_raster(alpha = .5, vjust = - 1, hjust = 1) + 
  geom_contour() +
  geom_spoke(radius = scales::rescale(train$Hm0, c(.01, .2)), angle = train$Dm*pi/180, arrow = arrow(length = unit(.05, 'cm')),alpha=0.3 ) +
  scale_fill_distiller(palette = "RdYlGn") + labs(fill = "Wave Height (m) \n", title = "April 6, 2010 ", subtitle = "train, 20:00") +
  coord_equal(expand = 0) +
  theme(legend.position = 'bottom',  legend.direction = 'horizontal',plot.title = element_text(hjust=0.5), plot.subtitle = element_text(hjust = 0.5))
test1<-ggmap(get_map(location = c(min(test$Lon), min(test$Lat), max(test$Lon), max(test$Lat)), zoom = 8),
      base_layer = ggplot(aes(x = Lon, y = Lat, z = Hm0,
                              fill = Hm0),
                          data = test)) +
  geom_raster(alpha = .5, vjust = - 1, hjust = 1) + 
  geom_contour() +
  geom_spoke(radius = scales::rescale(test$Hm0, c(.01, .2)), angle = test$Dm*pi/180, arrow = arrow(length = unit(.05, 'cm')),alpha=0.3 ) +
  scale_fill_distiller(palette = "RdYlGn") + labs(fill = "Wave Height (m) \n", title = "April 6, 2010 ", subtitle = "test, 20:00") +
  coord_equal(expand = 0) +
  theme(legend.position = 'bottom',  legend.direction = 'horizontal',plot.title = element_text(hjust=0.5), plot.subtitle = element_text(hjust = 0.5))
grid.arrange(train1,test1,ncol=2)
...
In our example we choose an exponential correlation, we run 2 chains parallelizing the procedure, (qui ci va una descrizione delle prior e il meccanismo adattativo)


```{r label="run1",message=FALSE, echo=TRUE, warning=FALSE, cache=TRUE}
...
```{r label="check1", message=FALSE, echo=TRUE, warning=FALSE, cache=FALSE}
check<- ConvCheck(mod)
check$Rhat
```
and the traceplots always remebering that \texttt{alpha} is a circular variable
```{r label="trace1", message=FALSE, echo=TRUE, warning=FALSE, cache=FALSE}
par(mfrow=c(2,2))
...
```{r label="krigstorm",message=FALSE, echo=TRUE, warning=FALSE, cache=TRUE}
Pred = WrapKrig(
   WrapSp_out = mod,
...
```
Once the estimates are obtained we compute the Average Prediction Error again following \cite{Jona2013}
```{r label=?"APEstorm", message=FALSE, echo=TRUE, warning=FALSE, cache=FALSE}
APE=APEcirc(
  real = test$Dmr,
   sim = Pred$Prev_out,
bycol=F
...
```{r label="calmacheck1", message=FALSE, echo=c(2,3,4,5), warning=FALSE, eval=TRUE}
check <- ConvCheck(mod)
check$Rhat
par(mfrow=c(2,2))
coda::traceplot(check$mcmc)
...
load("calma.RData")
check.up<- ConvCheck(mod.up)
check.up$Rhat
par(mfrow=c(2,2))
coda::traceplot(check.up$mcmc)
```

The convergence is reached and we can now move to the estimation on the test set.

```{r label="kigcalma", message=FALSE, echo=TRUE, warning=FALSE, cache=TRUE, eval=FALSE}
Pred.calm = WrapKrig(
   WrapSp_out = mod.up,
## The coordinates for the observed points
  coords_obs = coords.train,
...
```{r label="plot3",message=FALSE, echo=FALSE, warning=FALSE, cache=TRUE,eval=TRUE}
load("calma.RData")
APE=APEcirc(
  real = test$Dmr,
